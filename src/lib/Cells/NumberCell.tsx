...
export class NumberCell extends React.Component<CellProps, {}> {
    static Create(value: string, setValue: (value: any) => void): Cell {
        return {
            value,
            render: (cellProps) => <NumberCell {...cellProps} />,
            trySetValue: (v) => {
                let newValue = Number(v);
                (isNaN(newValue)) ? setValue(0) :
    ...
    render() {
        return <div {...this.props.attributes, { style: { ...this.props.attributes.style, textAlign: 'right', justifyContent: 'flex-end' } }}>
            {this.props.isInEditMode && <input style={{ textAlign: 'right', zIndex: zIndex.cellInput, width: this.props.attributes.style.width, height: this.props.attributes.style.height, border: 0, fontSize: 16, outline: 'none' }} ref={input => {
                if (input) {
                    input.focus()
